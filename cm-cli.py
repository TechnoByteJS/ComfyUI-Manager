...
def install_node(node_spec_str, is_all=False, cnt_msg=''):
    if '://' in node_spec_str:
        ...
        node_name, version_spec = node_spec
...
def update_node(node_spec_str, is_all=False, cnt_msg=''):
    ...
    return res.target(f'{node_name}@{res.target}')
...
def show_list(kind, simple=False):
    ...
    for k, v in custom_nodes.items():
        if 'cnr_latest' not in v:
            if len(v['files']) == 1:
                repo_url = v['files'][0]
                if repo_url not in unified_manager.unknown_inactive_nodes and repo_url not in unified_manager.unknown_active_nodes:
                    not_installed_unknown_nodes.append(v)
                else:
                    repo_unknown[repo_url] = v
    ...
    if kind in ['all', 'cnr', 'installed', 'disabled']:
        for k, v in unified_manager.cnr_inactive_nodes.items():
            ...
            cnr = unified_manager.cnr_map[k]
            processed[k] = "[    DISABLED   ] ", cnr['name'], k, cnr['publisher']['id'], v[0]
